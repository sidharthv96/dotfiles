...
function audioDeviceChanged(arg)
    ...
    elseif arg == 'dIn ' and os.time() - lastSetDeviceTime < 2 then
        ...
        if inputDevice:transportType() == 'Bluetooth' then
            internalMic = lastInputDevice or hs.audiodevice.findInputByName('MacBook Pro Microphone') or hs.audiodevice.findInputByName('Built-in Microphone') or hs.audiodevice.findInputByName('Internal Microphone')
